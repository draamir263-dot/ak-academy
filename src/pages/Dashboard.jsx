    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 dark:text-slate-100 p-4 md:p-8 font-sans pb-24 transition-colors duration-300" key={refreshKey}>
      <div className="max-w-5xl mx-auto">
        ...
        <header className="mb-8">
          <h1 className="text-3xl md:text-4xl font-extrabold text-slate-800 dark:text-slate-100">Performance Dashboard</h1>
        ...
        <div className={`rounded-2xl shadow-sm border p-6 mb-8 transition-colors ${isPremium ? 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800' : 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800'}`}>
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <div className="text-center md:text-left">
              <h2 className={`text-xl font-bold ${isPremium ? 'text-green-800 dark:text-green-300' : 'text-red-800 dark:text-red-300'}`}>{isPremium ? "⭐ Premium Account Active" : "🔒 Account Expired"}</h2>
              <p className="text-slate-600 dark:text-slate-400 mt-1 text-sm">{isPremium ? "Your premium access will expire in:" : "Your premium access has ended."}</p>
            </div>
            <div className="flex items-center gap-4">
              {isPremium && (
                <div className="text-center bg-white dark:bg-slate-800 px-4 py-2 rounded-xl shadow-sm border border-green-100 dark:border-slate-700">
                  <span className="text-2xl font-extrabold text-green-600 dark:text-green-400">{daysLeft}</span>
                  <p className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Days Left</p>
                </div>
              )}
              <Link to="/payment" className="bg-indigo-600 text-white px-5 py-3 rounded-lg font-semibold hover:bg-indigo-700 transition-colors text-sm flex items-center gap-2 shadow-md shadow-indigo-600/20">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5 10l7-7m0 0l7 7m-7-7v18" /></svg> {isPremium ? "Upgrade Plan" : "Recharge Now"}
              </Link>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 p-6 mb-8 flex flex-col md:flex-row items-center justify-between gap-8 transition-colors">
          <div className="flex items-center gap-6">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">Overall Accuracy</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Based on {totalUsed} solved MCQs</p>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-4 text-center w-full md:w-auto">
            <div className="px-4"><p className="text-2xl font-bold text-green-500">{totalCorrect}</p><p className="text-xs text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wide mt-1">Correct</p></div>
            <div className="px-4 border-l border-r border-slate-100 dark:border-slate-700"><p className="text-2xl font-bold text-red-500">{totalIncorrect}</p><p className="text-xs text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wide mt-1">Incorrect</p></div>
            <div className="px-4"><p className="text-2xl font-bold text-yellow-500">{totalFavourites}</p><p className="text-xs text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wide mt-1">Favorites</p></div>
          </div>
        </div>
              ...
              <div key={subject.name} className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden transition-colors">
                ...
                <div className="p-5 flex justify-between items-center cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors" onClick={() => setOpenSubject(isOpen ? null : subject.name)}>
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">{subject.name}</h3>
                ...
                {isOpen && (
                  <div className="border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 p-4 space-y-3">
                    {subject.chapters.map(chapter => {
                      ...
                      return (
                        <div key={chapter.name} className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-700 flex items-center justify-between gap-4">
                          <div className="flex-1 min-w-0">
                            <h4 className="font-bold text-slate-800 dark:text-slate-100 text-sm">{chapter.name}</h4>
                            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{cUsed} Attempted | {cCorrect} Correct | {cIncorrect} Incorrect</p>
                            <div className="flex items-center gap-2 mt-2">
                              {cUsed > 0 && cAcc < 60 ? (<span className="text-[10px] font-bold text-red-600 dark:text-red-400 bg-red-100 dark:bg-red-900/30 px-2 py-0.5 rounded">Weak</span>) : cUsed > 0 ? (<span className="text-[10px] font-bold text-green-600 dark:text-green-400 bg-green-100 dark:bg-green-900/30 px-2 py-0.5 rounded">Good</span>) : null}
                              <span className="text-[10px] font-semibold text-slate-400 dark:text-slate-500">Completion: {progressWidth}%</span>
                            </div>
                          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white dark:bg-slate-800 border-t border-slate-100 dark:border-slate-700 flex justify-around py-3 px-5 rounded-t-2xl shadow-2xl z-50 transition-colors duration-300">
        ...
        <Link to="/dashboard" className="flex flex-col items-center text-indigo-600 dark:text-indigo-400">
